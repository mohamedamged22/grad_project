...
class _TouristTripDetailsViewState extends State<TouristTripDetailsView> {
  ...
  void initState() {
    super.initState();
    _detailsCubit =
        sl<TouristTripDetailsCubit>()..fetchTripDetails(tripId: widget.trip.id);
    ...
    return BlocProvider.value(
      ...
      child: BlocBuilder<TouristTripDetailsCubit, TouristTripDetailsState>(
        builder: (context, state) {
          ...
          final coverUrl =
              details?.normalizedCoverImageUrl ??
              widget.trip.normalizedImageUrl;
          final dateRange = details?.dateRange ?? '';
          final duration =
              (details?.tourDuration ?? widget.trip.duration).toString().trim();
          ...
          final status = details?.status ?? widget.trip.creatorName;
          final meetingPoint = details?.meetingPoint ?? '';
          final priceText =
              details?.formattedPrice ?? widget.trip.formattedPrice;
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color:
                            isDark ? const Color(0xFF1A2833) : AppColor.white,
                        borderRadius: BorderRadius.circular(10.r),
                        border: Border.all(
                          color:
                              isDark
